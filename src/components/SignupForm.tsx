  ...
  if (status === "done") {
    return (
      <div className="rounded-2xl border border-line bg-surface p-7 text-center">
        <p className="font-semibold">{t.confirmEmailTitle}</p>
        <p className="mt-1.5 text-sm text-ink-soft">{t.confirmEmailBody}</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-line bg-surface p-7">
      ...
      <p className="mt-6 text-center text-sm text-ink-faint">
        {t.hasAccount}{" "}
        <Link href={localizedHref("/login", locale)} className="font-medium text-ink underline">
          {t.signInLink}
        </Link>
      </p>
    </div>
